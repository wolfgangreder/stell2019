 ...
 */
@Messages({"# {0} - class name from MRE",
...
public final class NbFormatter extends java.util.logging.Formatter
{
                     ...
                     Set<Throwable> beenThere)
  {
    ...
    if (record.getThrown() != null && record.getLevel().intValue() != 1973) {
      ...
      try {
        ...
        printStackTrace(record.getThrown(),
                        pw);
        pw.close();
        sb.append(sw.toString());
      } catch (Exception ex) {
      }
      LogRecord[] arr = extractDelegates(sb,
                                         record.getThrown(),
                                         beenThere);
      if (arr != null) {
        for (LogRecord r : arr) {
      ...
      specialProcessing(sb,
                        record.getThrown(),
                        beenThere);
    }
  }

  private static void addLoggerName(StringBuilder sb,
  ...
  private static LogRecord[] extractDelegates(StringBuilder sb,
                                              Throwable t,
                                              Set<Throwable> beenThere)
  {
    if (!beenThere.add(t)) {
    ...
    return extractDelegates(sb,
                            t.getCause(),
                            beenThere);
  }

  private void specialProcessing(StringBuilder sb,
                                 Throwable t,
                                 Set<Throwable> beenThere)
  {
    ...
    if (t instanceof MissingResourceException) {
      ...
      if (cn != null) {
        LogRecord rec = new LogRecord(Level.CONFIG,
                                      null);
        rec.setResourceBundle(NbBundle.getBundle(NbFormatter.class));
   ...
   */
  private static void doPrintStackTrace(PrintWriter pw,
                                        Throwable t,
                                        Throwable higher,
                                        int depth)
  {
    ...
    try {
      if (t.getClass().getMethod("printStackTrace",
                                 PrintWriter.class).getDeclaringClass() != Throwable.class) { // NOI18N
  ...
  static void registerCatchIndex(Throwable t,
                                 int index)
  {
    catchIndex.put(t,
                   index);
