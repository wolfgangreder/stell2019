...
public final class CanSocket implements Closeable
{
  ...
  private static String getLibFile(String libName)
  {
    StringBuilder tmp = new StringBuilder("/lib/");
    String os = System.getProperty("os.name").toLowerCase();
    tmp.append(os);
    tmp.append('/');
    tmp.append(System.getProperty("os.arch").toLowerCase());
    tmp.append("/lib");
    tmp.append(libName);
    tmp.append(".so");
    return tmp.toString();
          ...
          throws IOException
  {
    ...
    final String fileName = getLibFile(libName);
  ...
  private static native void _sendFrame(final int fd,
                                        final int canif,
                                        final int canid,
  ...
  private static native void _setsockopt(final int fd,
                                         final int op,
                                         final int stat) throws IOException;

  private static native int _getsockopt(final int fd,
                                        final int op)
          throws IOException;
  ...
  public final static class CanInterface implements Cloneable
  {
    ...
    public CanInterface(final CanSocket socket,
                        final String ifName)
            throws IOException
    {
      this._ifIndex = _discoverInterfaceIndex(socket._fd,
                                              ifName);
      this._ifName = ifName;
    }
    ...
    public String resolveIfName(final CanSocket socket)
    {
      if (_ifName == null) {
        try {
          _ifName = _discoverInterfaceName(socket._fd,
                                           _ifIndex);
        } catch (IOException e) {
          /* EMPTY */ }
      }
    ...
    protected Object clone()
    {
      return new CanInterface(_ifIndex,
                              _ifName);
    }
  ...
  public final static class CanFrame implements Cloneable
  {
    ...
    public CanFrame(final CanInterface canIf,
                    final CanId canId,
                    byte[] data)
    {
      this.canIf = canIf;
